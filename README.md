...
#### 🚀 Quickstart: Windows Automated 1-Click Installer
For a seamless Windows installation without needing manual Conda/Python configuration:
1. Double-click **`WINDOWS_INSTALL.bat`**.
   - Automatically detects CPU & NVIDIA GPU/CUDA hardware.
   - Sets up Python (or downloads a portable embedded Python 3.10 if not present).
   - Installs `uv`, `PyTorch` (CUDA 11.8 or CPU), and all dependencies.
   - Generates Desktop and Start Menu shortcuts.
2. Double-click **`LAUNCH.bat`** (or your Desktop shortcut) to start the interactive application menu or pass CLI arguments.
3. For in-depth installation options, see [README-INSTALLATION.md](file:///c:/$RMYHTJ4/AudioBookSlides-main/README-INSTALLATION.md).

---

#### Manual Installation (Conda / Mamba)
...
**Windows (Manual)**:
<details><summary>Manual Windows Installation Steps (click to expand)</summary>
